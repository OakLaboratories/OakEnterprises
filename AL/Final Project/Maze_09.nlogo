...
enemies-own [species health speed strength]
bosses-own [health]
...
to create-power-ups
  if power-up-state < 4 [
    if 1 > random 100 [
      ask n-of 1 patches with [pcolor = 16.6 ] [
        sprout 1 [
          set breed powerUps 
  ...
  ask powerUps with [function = "strength"] [set shape "wolf" set size 2 set color yellow]
  ask powerUps with [function = "health"] [set shape "cow" set size 2 set color blue]
  ask powerUps with [function = "speed"] [set shape "fish" set size 2 set color green]
end
...
to go
  ...
  ;Enemies
  ask players [if not any? enemies in-radius 3 [set health health + 1.5]]
