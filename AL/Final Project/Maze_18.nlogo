...
to show_map
  if map-on? = false [
    ...
    set map-on? true
    ask enemies with [species = "fast"] [set shape one-of ["pterodactyl" "centaur" "viking" "bartender" "skateboarder" "ufo side" "robot"]]
    ask enemies with [species = "strong"] [set shape one-of ["st tiger" "cyclops" "troll" "capitalist" "homeless" "zombie" "elderly"]]
    ask enemies with [species = "long-range"] [set shape one-of ["neanderthal" "medusa" "dark elf" "general" "thug" "alien" "terminator"]]
    ask players [set shape one-of ["cavemen" "spartan" "knight" "worker" "student" "survivor" "cyborg"]]
    wait 2 
  ]
  ifelse timer > 25
    [set map-on? false]
...
to setup_3
  ...
  ask patches with [pcolor != 16.6 and pcolor != 72.5] [set pcolor one-of [42 black]]
  set power-up-state 0
  set level 3
  ;Player Setup
  create-players 1 [
...
to go_8
  ...
  ask enemies [if health < 3 [die]]
  random_enemies
  ask enemies with [species = "fast" and age = "young"] [set shape "glitch"]
  ask enemies with [species = "strong" and age = "young"] [set shape "glitch"]
  ask enemies with [species = "long-range" and age = "young"] [set shape "glitch"]
  show_map
  ;Players
  health_check
  if restart = true [
