...
to direction [degree]
  set heading degree
  if [pcolor] of patch-ahead 1 = red or [pcolor] of patch-ahead 1 = blue [fd speed]
end

to attack
  if any? enemies in-radius 5
  [ask min-one-of enemies in-radius 5 [distance myself] [set health health - 2]]
end
...
to setup-BC-Era
  ...
  cro 1 [setxy -16 -8 
    set heading 90
    repeat 5 [ask patch-here [set pcolor red] fd  1]
    set heading 0
    repeat 15 [ask patch-here [set pcolor red] fd  1]
    set heading 90
    repeat 3 [ask patch-here [set pcolor red] fd  1]
    set heading 0
    repeat 3 [ask patch-here [set pcolor red] fd  1]
    set heading 90
    repeat 5 [ask patch-here [set pcolor red] fd  1]
    set heading 180
    repeat 24 [ask patch-here [set pcolor red] fd  1]
    set heading 90
    repeat 5 [ask patch-here [set pcolor red] fd  1]
    set heading 0
    repeat 20 [ask patch-here [set pcolor red] fd  1]
    set heading  270
    repeat 2 [ask patch-here [set pcolor red] fd  1]
    set heading 0
    repeat 3 [ask patch-here [set pcolor red] fd  1]
    set heading 90
    repeat 10 [ask patch-here [set pcolor red] fd  1]
    set heading 180
    repeat 20 [ask patch-here [set pcolor red] fd  1]
    set heading 270
    repeat 3 [ask patch-here [set pcolor red] fd  1]
    set heading 0
    repeat 9 [ask patch-here [set pcolor red] fd  1]
    set heading 270
    repeat 3 [ask patch-here [set pcolor red] fd  1]
    set heading 180
    repeat 20 [ask patch-here [set pcolor red] fd  1]
    set heading 90
    repeat 12 [ask patch-here [set pcolor red] fd  1]
    ask patch-here [set pcolor blue]
    die
    ]
  
  ask patches with [pcolor != red and pcolor != blue] [set pcolor one-of [ 54 55 56  64 65 66 ]]
  create-players 1 [
    set health 100
    setxy -16 -8
    set shape "cavemen"
...
to setup-tRex-boss
  ...
  repeat 6 [ ask patches with [pxcor = random-pxcor and pycor = random-pycor] [  set state "human-health"]]
  repeat 6 [ ask patches with [pxcor = random-pxcor and pycor = random-pycor] [  set state "trex-health"]]
  repeat 1 [ ask patches with [pxcor = random-pxcor and pycor = random-pycor] [  set state "bad-human-health"]]
...
to go
  ask players [set speed 1]
  ask enemies [if any? players in-radius 10 [ask players [set speed .3 ]
      face player 1 fd .5 if any? players-here [ask players-here [set health health - 1] face player 1 rt 180 bk 3]]]
    ask players [if health < 20 [die]]
    ask enemies [if health < 3 [die]]
