...
globals [movement power-up-state]
breed [players player]
breed [enemies enemy]
breed [bosses boss]
breed [powerUps powerUp]

players-own [health speed]
enemies-own [species health]
bosses-own [health]
...
to direction [degree colour]
  set heading degree
  if [pcolor] of patch-ahead 1 = colour or [pcolor] of patch-ahead 1 = 72.5 [fd speed]
end
...
to attack_fast
  if any? enemies in-radius 5 with [state = "strong"] [
    ask min-one-of enemies with [state = "strong"] [distance myself] [set health health - 4]
  ]
  if any? enemies in-radius 5 with [state = "fast"] [
    ask min-one-of enemies with [state = "fast"] [distance myself] [set health health - 2]
  ]if any? enemies in-radius 5 with [state = "long-range"] [
    ask min-one-of enemies with [state = "long-range"] [distance myself] [set health health - 1]
  ]
end

to attack_strong
  if any? enemies in-radius 5 [
    ask min-one-of enemies with [state = "strong"] [distance myself] [set health health - 2]
    ask min-one-of enemies with [state = "fast"] [distance myself] [set health health - 1]
    ask min-one-of enemies with [state = "long-range"] [distance myself] [set health health - 4]
  ]
end

to attack_long-range
  if any? enemies in-radius 5 [
    ask min-one-of enemies with [state = "strong"] [distance myself] [set health health - 1]
    ask min-one-of enemies with [state = "fast"] [distance myself] [set health health - 4]
    ask min-one-of enemies with [state = "long-range"] [distance myself] [set health health - 2]
  ]
end
...
to create-power-ups
  ...
  [
  if 1 > random 100 [
    ask n-of 1 patches with [pcolor = 16.6 ]  [sprout 1 [ set breed powerUps ]]
...
to go
  ...
  ask enemies [if any? players in-radius 10 [
      ask players [set speed .5]
      face player 0 
      fd .5 
      if any? players-here [
        ask players-here [set health health - 1] 
        face player 0 
        rt 180 
        bk 3
        ]
  ...
  ask players [if any? powerUps-here [set health 100 set color red set size 5 wait .1 ask powerUps [die]] ]
  ask players [if health < 20 [die]]
  ask enemies [if health < 3 [die]]
  if count players = 0 [
    wait 2 
